---
title: "Metabolite_survival: polar metabolites"
...
write.csv(out_dat2, "Survival_stand.gee_batch5.pc_genesis.03.30.2023.csv", row.names=F)
```
...
out_dat1_annot <- data.frame(annot.dat, out_dat2[match(annot.dat$Compound.Name, out_dat2$metabolite),]) %>%
  arrange(metab_pval)
write.csv(out_dat1_annot, "annotated_Survival_stand.gee_batch5.pc_genesis.03.30.2023.csv")
```

# Plot
```{r}
out_dat1_annot <- read.csv( "annotated_Survival_stand.gee_batch5.pc_genesis.03.30.2023.csv", header=T)
...
# metab.names <- str_replace(as.character(out_dat1_annot$Standardized.name[1:nsign]), "/"," ")
...
# Plot for paper
 tryptophan = metab26
 Linolenic acid metab84
 Ergothioneine metab40
 THREITOL metab186
 Tartaric acid metab105
```{r}
out_dat1_annot <- read.csv( "annotated_Survival_stand.gee_batch5.pc_genesis.03.30.2023.csv", header=T)
...
out_dat1_annot <- out_dat1_annot[order(out_dat1_annot$metab_pval), ]
 ...
 analysis.final.no.missing.dat <- read.csv("analysis.final.no.missing.dat.batch5.csv", header=T)
require(survminer)
metab <- paste0("metab",c(26, 84,40,186,105))
metab.names <- c("Tryptophan", "Octadecatrienoic acid", "Ergothioneine", "Threitol","Tartaric acid")
...
for(i in metab){
  ...
  fit <- survfit(Surv(FU, Status) ~ group, data = analysis.final.no.missing.dat) 
    p<-ggsurvplot(fit, data = analysis.final.no.missing.dat,
      ...
      labs(title=metab.names[j])
    print(p)
  png(paste0("plot_dir/", metab.names[j], ".07.2023.png"), width = 480, height = 480)
  print(p)
  dev.off()
